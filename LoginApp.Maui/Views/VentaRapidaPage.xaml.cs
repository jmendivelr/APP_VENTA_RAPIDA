...
public partial class VentaRapidaPage : ContentPage
{
    ...
    public VentaRapidaPage()
    {
        InitializeComponent();
        //detectorImagen.Options = new  BarcodeReaderOptions
        //{
        ...
        listaProductos.CollectionChanged += (sender, e) =>
        {
            if (e.NewItems != null)
            {
                foreach (ProductoViewModel nuevoProducto in e.NewItems)
                {
            ...
            if (e.OldItems != null)
            {
                foreach (ProductoViewModel viejoProducto in e.OldItems)
                {
    ...
    private void ProductoViewModel_PropertyChanged(object sender, PropertyChangedEventArgs e)
    {
        ...
        {
            ActualizarTotalPreciosGenerales();
        }else if(e.PropertyName == nameof(ProductoViewModel.Precio2Seleccionado))
        {
            ActualizarTotalPreciosGenerales();
        }else if (e.PropertyName == nameof(ProductoViewModel.Cantidad))
        {
            ActualizarTotalPreciosGenerales();
...

    {
        ...
        Navigation.PushAsync(new BuscarProductosPage());
    }
    ...
    private async void CerrarPopupFinalizaVenta(object sender, EventArgs e)
    {
        ...
        VentaData ventaData = new VentaData
        {
            Faccab = new FACCAB
            {
                ...
                CFIGV = ((TotalPreciosSeleccionadosGeneral/1.18m) - TotalPreciosSeleccionadosGeneral),             // IGV (calcular como TOTAL / 1.18)
    ...
    //private async void EscanearCodigoBarras_Clicked(object sender, EventArgs e)
    //{
    //    try
    //    {
    ...
    //    if (result != null && !string.IsNullOrWhiteSpace(result.Text))
    //    {
    //        Aquí puedes manejar el resultado del escaneo, por ejemplo, mostrar el código de barras en una etiqueta o realizar alguna acción relacionada.
    //         result.Text contiene el valor del código de barras escaneado.
    //         Puedes redirigir a la página anterior o realizar otras acciones según tus necesidades.
    //        await DisplayAlert("Resultado del escaneo", $"Código de barras escaneado: {result.Text}", "OK");
    //    }
    //}
    //private async void MiBoton_Clicked(object sender, EventArgs e)
    //{
    //    try
    //    {
    //        var result = await Xamarin.Essentials.BarcodeScanner.ScanAsync();

    //        if (result != null)
    //        {
    //            // Aquí puedes manejar la información del código de barras escaneado
    //            string scannedText = result.Text;
    //            // Puedes mostrar el resultado en una alerta o hacer lo que necesites
    //            await DisplayAlert("Código de Barras Escaneado", scannedText, "OK");
    //        }
    //    }
    //    catch (Exception ex)
    //    {
    //        // Manejar excepciones aquí
    //        await DisplayAlert("Error", $"Error al escanear el código de barras: {ex.Message}", "OK");
    //    }
    //}
    //private void detectorImagen_BarcodesDetected(object sender, BarcodeDetectionEventArgs e)
    //{
